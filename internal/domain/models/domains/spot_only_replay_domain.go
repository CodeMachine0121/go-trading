...
// else.
//
// A caller with only one of the two declarations to make passes nothing for the other,
// and nothing is what both of them most often are.
func NewSpotOnlyReplayDomain(
	...
	if normalizedDeclaration != "" &&
		...
		return SpotOnlyReplayDomain{}, fmt.Errorf("%w", ErrSpotOnlyTradingMode)
	}
	...
	if !declaredLeverageMultiplier.IsZero() &&
		...
		return SpotOnlyReplayDomain{}, fmt.Errorf("%w", ErrLeverageMultiplierBelowOne)
	}
	...
	if declaredLeverageMultiplier.GreaterThan(noBorrowing) {
		return SpotOnlyReplayDomain{}, fmt.Errorf("%w", ErrSpotOnlyBorrowing)
	}
