...
func lookbackCountParameter(name string, defaultValue float64) dto.StrategyScriptParameterWriteDto {
	return dto.StrategyScriptParameterWriteDto{Name: name, Kind: "lookbackCount", DefaultValue: defaultValue}
}

func numberParameter(name string, defaultValue float64) dto.StrategyScriptParameterWriteDto {
	return dto.StrategyScriptParameterWriteDto{Name: name, Kind: "number", DefaultValue: defaultValue}
}

func booleanParameter(name string, defaultValue float64) dto.StrategyScriptParameterWriteDto {
	return dto.StrategyScriptParameterWriteDto{Name: name, Kind: "boolean", DefaultValue: defaultValue}
}

func parametersOf(t *testing.T, declared ...dto.StrategyScriptParameterWriteDto) domains.StrategyScriptParametersDomain {
	t.Helper()

	parameters, buildError := domains.NewStrategyScriptParametersDomain(declared)
...
func TestDeclaringParametersRefusesWhatCannotBeOne(t *testing.T) {
	testCases := []struct {
		name             string
		declared         []dto.StrategyScriptParameterWriteDto
		expectedFragment string
	}{
		{
			name:             "同一支策略腳本內名稱不得重複",
			declared:         []dto.StrategyScriptParameterWriteDto{lookbackCountParameter("期數", 20), numberParameter("期數", 2)},
			expectedFragment: "重複",
		},
		{
			name:             "名稱不得為空白",
			declared:         []dto.StrategyScriptParameterWriteDto{lookbackCountParameter("   ", 20)},
			expectedFragment: "不得為空白",
		},
		{
			name:             "回看根數必須大於零",
			declared:         []dto.StrategyScriptParameterWriteDto{lookbackCountParameter("期數", 0)},
			expectedFragment: "大於零的整數",
		},
		{
			name:             "回看根數必須是整數",
			declared:         []dto.StrategyScriptParameterWriteDto{lookbackCountParameter("期數", 20.5)},
			expectedFragment: "大於零的整數",
		},
		{
			name:             "名稱太長",
			declared:         []dto.StrategyScriptParameterWriteDto{lookbackCountParameter(strings.Repeat("長", 65), 20)},
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			_, buildError := domains.NewStrategyScriptParametersDomain(testCase.declared)

			require.ErrorIs(t, buildError, domains.ErrStrategyScriptParameterValidation)
...
func TestMaximumLookbackCountLooksOnlyAtLookbackCounts(t *testing.T) {
	testCases := []struct {
		...
		{name: "一個都沒有就是零", declared: nil, expected: 0},
		{
			...
			declared: []dto.StrategyScriptParameterWriteDto{numberParameter("倍數", 999)},
			expected: 0,
		},
		{
			name:     "只看最大的那一個",
			declared: []dto.StrategyScriptParameterWriteDto{lookbackCountParameter("快線", 20), lookbackCountParameter("慢線", 100), lookbackCountParameter("中線", 50)},
			expected: 100,
		},
		{
			name:     "數值再大也不算進來",
			declared: []dto.StrategyScriptParameterWriteDto{lookbackCountParameter("期數", 20), numberParameter("倍數", 5000)},
			expected: 20,
		},
	}
...
func TestApplyingThisRunsValues(t *testing.T) {
	...
	t.Run("給了沒有宣告的名稱就整次拒絕", func(t *testing.T) {
		...
		require.ErrorIs(t, applyError, domains.ErrStrategyScriptParameterValidation)
		assert.Contains(t, applyError.Error(), "週期")
	})

	t.Run("給的回看根數不合法就整次拒絕", func(t *testing.T) {
		_, applyError := parametersOf(t, lookbackCountParameter("期數", 20)).
			Applying([]dto.StrategyScriptParameterValueDto{{Name: "期數", Value: 0}})
	...
	t.Run("套值之後最大回看根數跟著變——要拿幾根就是照這個算的", func(t *testing.T) {
		applied, applyError := parametersOf(t, lookbackCountParameter("期數", 20)).
			Applying([]dto.StrategyScriptParameterValueDto{{Name: "期數", Value: 100}})
...
func TestABooleanIsOneNumberReadAsYesOrNo(t *testing.T) {
	...
	t.Run("這一次改成別的數字，收回來的仍然是零或一", func(t *testing.T) {
		...
		applied, applyError := parameters.Applying(
			[]dto.StrategyScriptParameterValueDto{{Name: "要濾掉假突破", Value: 2}})
	...
	t.Run("零不會被當成不合法——是非沒有大於零這回事", func(t *testing.T) {
		_, buildError := domains.NewStrategyScriptParametersDomain(
			[]dto.StrategyScriptParameterWriteDto{booleanParameter("只看多方", 0)})
