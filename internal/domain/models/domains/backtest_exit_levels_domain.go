...
// PricesFrom places both exits around one entry price.
//
...
//
// This settles *where* the two prices are. Whether a candle reached one of them is the
...
func (backtestExitLevelsDomain BacktestExitLevelsDomain) PricesFrom(
	...
	if exitPricesVo.HasStopLoss {
		exitPricesVo.StopLossPrice = movedBy(
			entryPrice, backtestExitLevelsDomain.stopLoss, isShort)
	}
