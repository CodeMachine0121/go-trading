...
tradingMode 答兩個各自獨立的問題——做得了哪一邊、借不借得到錢——四個取值就是那兩個問題的四種合法組合
（做得了空卻借不到錢不存在：放空本來就要先借到東西才賣得出去）。不給就是 longShort：
  longShort（預設）→ 兩邊都做、借得到錢：賣出把多倉平掉，並在同一根反手開空；空手時賣出直接開空。
  spot            → 只做多、借不到錢：賣出就平倉把錢收回來、之後空手等下一個買點；空手時賣出什麼都不做，永遠不開空。
  leveragedLong   → 只做多、借得到錢：進出場與 spot 一字不差，差別只有它開得了槓桿。
  shortOnly       → 只做空、借得到錢：leveragedLong 的鏡像——賣出開空倉，買入就平倉把錢收回來、之後空手等下一個賣點；空手時買入什麼都不做，永遠不開多。
...
他的機器人也存不進大於 1 倍的部位。
他說他這段時間只想做空、不要叫他做多時要給 shortOnly。絕對不要拿 longShort 配一個永遠不成立的
買入條件去湊：那樣跑得出來，但那份規則會寫著兩邊都做、實際上只做空，下一個讀到它的人（包括三個月
後的他自己）沒有任何方法看出來。
場所不決定模式，做哪一邊才決定。「我在幣安永續」「我開合約」只講了場所——longShort、leveragedLong
與 shortOnly 都跑在那裡，那句話一個都沒排除掉。沒問出他做哪一邊之前不要猜：該給 leveragedLong
卻給了 spot，他會被拒絕，至少他知道；該給 leveragedLong 或 shortOnly 卻給了 longShort（也就是
不給，因為那是預設），他每一次反向信號都會被反手，而成績單照樣跑得出來、看起來完全合理，他不會發現。
