...
| `POST` | `/k-candles/history` | 同步一段歷史（body 給 `symbol` 與 `lookbackDays`）：向來源問整段，**只補系統沒有的，已經有的不覆蓋**。粒度固定一分鐘一根。回溯天數不在 1 到上限之間回 `400` 並說出上限，沒登錄過的代號回 `404` |
...
**這個系統自己**問不到（讀不到資料庫之類）回 `502`（那值得晚點再試一次）。

**行情來源問不到不是整次失敗。** 那回 `200`，而那一檔的 `fetchFailureReason`
寫著發生了什麼、`wasAsked` 是 `false`。一檔問不到是關於那一檔的事實，
不是這次請求失敗了——報告本來就有一格專門講它，而一次問好幾檔時也不該
讓其中一檔拖垮其餘的。
