...
| `CONTRACT_MARKET_DATA_MARK_PRICE_URL` | 幣安標記價格網址 | 永續合約的**標記價格**來源位址。它是另一支位址，不是同一支的參數——一根合約 K 線由這兩份答案合成 |
...
| `CONTRACT_MARKET_DATA_REQUESTS_PER_MINUTE` | `300` | 每分鐘最多向合約場所打幾次。**與現貨那份各自計算**，因為兩個場所各記各的額度；共用一份等於各只用到一半。預設是現貨的一半，因為這裡每一根要問兩次 |
...
| `POST` | `/contract-k-candles` | 新增一根**合約** K 線；同交易標的同起始時間即覆蓋。標記價格與成交筆數必填 |
...
| `GET` | `/contract-trading-symbols` | 列出系統認得的每一個**合約**標的：已登錄的加上實際有合約 K 線的，去重、依名稱排序 |
...
| 成交筆數 | 不記 | 每一根都記（零是合法值） |
| 每一根要問幾次來源 | 一次 | **兩次**（價量、標記價格），依起始時間對齊 |
| 欄位可不可以留白 | 成交額等三項可以（遷就不提供的市場） | **每一欄都不可以**（只有一個來源，而它每一項都給） |
| 代號宇宙 | 約 3700 個 | 約 900 個，其中**四百多個只有合約才有** |

### 一根合約 K 線由兩份資料合成，缺一不存

價量與標記價格是**分兩次**向來源問的，依起始時間對齊成一根。呼叫端只發一次請求——
問了兩次、對齊、丟掉佔位的零，全部藏在來源那一層。

**某一分鐘的標記價格沒問到，那一根就整根不存**，跳過並留下紀錄，同批其他完整的照常
...
即時跟盤、把合約 K 線合併成更粗的刻度、**資金費率**、**未平倉量**，以及讓指標計算與
回測吃得到合約 K 線——都還沒有。下游（回測、指標、交易策略、機器人）目前完全不知道
這件事發生過。
