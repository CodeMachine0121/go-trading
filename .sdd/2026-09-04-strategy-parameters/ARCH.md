...
```
InputCandleCount() = 呼叫端要看的根數 + max(0, MaximumLookbackCount() − 1)
```

**那個「減一」只在真的有東西要回看時才成立。** 回看二十根的算式要到第二十根才產出第一個值，
所以多花十九根；而**完全沒有宣告回看根數的算式一根都不多花**——不是少花一根。

> 本文件先前寫的是 `+ MaximumLookbackCount() − 1`，並宣稱「同一條式子涵蓋兩種情形」。
> **那句話是錯的**：沒有回看根數時它會算成「要看的根數減一」，少拿一根。
> 既有的計算測試在實作當下就把它擋了下來。
