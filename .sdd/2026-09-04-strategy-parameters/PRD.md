...
- **Primary Role(s):** 自己寫策略腳本、也自己看行情的人（本專案作者本人）。
- **Usage Context:**
...
### US-01 — 策略腳本帶著自己的旋鈕 [priority: P0]

**As a** 寫策略腳本的人，**I want** 策略腳本記著它有哪幾個可以調的東西，
**so that** 我改數字時不必去動算式。

```gherkin
Scenario: 帶著參數的策略腳本被完整記住
  Given 一支策略腳本宣告「期數」是回看根數、預設 20
  And 同一支策略腳本宣告「倍數」是數值、預設 2
  When 建立這支策略腳本
  Then 兩個參數連同它們的種類與預設值都被記住

Scenario: 讀回來時參數原樣還在
  Given 一支已經建立、帶著「期數」與「倍數」的策略腳本
  When 讀取這支策略腳本
  Then 兩個參數連同種類與預設值原樣回來

Scenario: 一個參數都不宣告的策略腳本照常運作
  Given 一支策略腳本一個參數都沒有宣告
  When 建立它並執行一次計算
  Then 建立成功
  And 計算照常進行，與沒有這項功能時完全相同

Scenario: 參數名稱不得重複
  Given 一支策略腳本的兩個參數都叫「期數」
  When 建立這支策略腳本
  Then 建立被拒絕
  And 說明參數名稱不得重複

Scenario: 參數名稱不得為空白
  Given 一支策略腳本的某個參數名稱是空白
  When 建立這支策略腳本
  Then 建立被拒絕
  And 說明參數名稱不得為空白

Scenario: 參數名稱前後的空白不予保留
  Given 一支策略腳本的某個參數名稱前後帶著空白
  When 建立這支策略腳本
  Then 記住的是去掉前後空白之後的名字

Scenario: 回看根數必須大於零
  Given 一支策略腳本宣告「期數」是回看根數、預設 0
  When 建立這支策略腳本
  Then 建立被拒絕
  And 說明回看根數必須大於零

Scenario: 參數種類只有兩種
  Given 一支策略腳本宣告某個參數的種類既不是回看根數也不是數值
  When 建立這支策略腳本
  Then 建立被拒絕
  And 說明參數種類只有回看根數與數值兩種

Scenario: 數值不限正負與小數
  Given 一支策略腳本宣告「偏移」是數值、預設 -1.5
  When 建立這支策略腳本
...
Scenario: 沒有回看根數時就是那一段的根數
  Given 要看的那一段裡有 12 根
  And 這支策略腳本一個回看根數參數都沒有
  When 執行計算
  Then 拿 12 根 K 線餵給算式

Scenario: 好幾個回看根數時只看最大的
  Given 這支策略腳本的回看根數參數是 20、50、100
  And 要看的那一段裡有 12 根
  When 執行計算
...
Scenario: 這一次給的值蓋過宣告的預設值
  Given 一支策略腳本宣告「倍數」是數值、預設 2
  And 這一次給「倍數」的值是 2.5
  When 執行計算
  Then 算式取到 2.5

Scenario: 算式取用沒有宣告的名字就是這次計算失敗
  Given 一支策略腳本宣告的參數叫「週期」
  And 算式取用的名字是「期數」
  When 執行計算
  Then 這一次計算失敗
  And 說明「期數」這個名字沒有被宣告
...
Scenario: 給了沒有宣告的參數名稱就整次拒絕
  ...
  When 執行計算
  Then 整次計算被拒絕
  And 說明「週期」不是這支策略腳本的參數

Scenario: 給的回看根數不合法就整次拒絕
  Given 一支策略腳本宣告「期數」是回看根數
  And 這一次給「期數」的值是 0
  When 執行計算
  Then 整次計算被拒絕
  And 說明回看根數必須大於零
...
- **一支策略腳本記著它有哪些參數**：每個參數有參數名稱、參數種類、參數預設值。
- **參數種類只有兩種**：回看根數（大於零的整數）與數值（任何數字）。
- **同一支策略腳本內參數名稱不得重複**；不得為空白；前後空白不予保留。
...
系統這一側沒有畫面。對呼叫端而言，一支策略腳本多回覆一份參數宣告，
...
- **既有策略腳本會壞掉，而這是刻意接受的。** 系統仍在開發階段、尚未上線，
