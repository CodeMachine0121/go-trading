...
- **Guiding principle:**
  **應用層每個用例只呼叫 `KCandleService` 一次。**
  所有規則檢查、覆蓋策略腳本、排序與筆數上限都封裝在 domain 內部；`KCandleApplication`
...
- **Patterns applied & why:**
  ...
  - **刻意不套用 StrategyScript 於 K 線長度**：目前只有一種長度，先蓋策略腳本架構屬於臆測性通用化。
...
- **Known debt / deferred:**
  ...
  - **沒有分頁**：超過上限一律拒絕而非分頁。
    **該重看的訊號**：策略腳本程式頻繁需要超過上限的區間，被迫自行切段時。
