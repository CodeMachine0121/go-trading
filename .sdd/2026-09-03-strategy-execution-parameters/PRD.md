...
- **Problem Statement:**
  一支策略腳本現在記著五樣東西：策略腳本名稱、指標算式、指標值種類、彙總刻度與計算根數。
  前三樣講的是「這套算法是什麼」，後兩樣講的是「這一次要餵它吃哪一段資料」——
  那是執行當下才知道的事，不是算法的一部分。

  代價有兩層。第一層是策略腳本清單被同一套算法的取數變體塞滿：
  同一支「二十根均線」想在一小時刻度看一次、再在五分鐘刻度看一次，
  就得存成兩支算式一字不差、只有取數不同的策略腳本。
  第二層更嚴重——那兩樣**從來沒有生效過**。
  指標計算至今只讀原始的五分鐘 K 線、固定拿最新的那幾根，
  從未問過策略腳本記著的彙總刻度與計算根數是多少。存進去的是一份沒有人讀的願望。

- **Expected Outcome:**
  策略腳本回到它本來的樣子：一套算法。策略腳本清單上的每一列都是一套**不同的算法**，
  而不是同一套算法的不同看法。
  同時，同一支策略腳本能在五種粗細、任意一段時間上執行，
  且每一次計算都真的照它說的粗細與根數取數——不再有寫下來卻沒人讀的設定。

- **Out of Scope:**
  - 在圖表上套用策略腳本、把指標值畫出來——那是操作介面的事。
  - 把交易標的記在策略腳本身上——市場仍然是執行時才指定的。
  - 新增「這支算法至少需要幾根」這類欄位——刻意不做。
  - 策略腳本的執行紀錄或計算結果留存——策略腳本是食譜，值每次跑每次算。
  - 既有策略腳本的彙總刻度與計算根數的遷移——它們沒有地方可去，直接消失。
...
- **Primary Role(s):** 寫策略腳本並拿它去算的人（本專案作者本人），
  以及代替他發問的操作介面——尤其是圖表畫面，它會在使用者拉遠拉近時
  以當下的粗細與看得到的那一段時間去執行同一支策略腳本。
- **Usage Context:** 一次互動就要有一次回覆，屬於即時的讀取，不是批次作業。
  同一支策略腳本在一次瀏覽中可能以好幾種粗細被執行好幾次。

---

## 3. User Stories & Acceptance Criteria

### US-01 — 策略腳本只記住一套算法 [priority: P0]

**As a** 寫策略腳本的人，**I want** 一支策略腳本只記著名稱、算式與指標值種類，
**so that** 我的策略腳本清單上每一列都是一套不同的算法，而不是同一套算法的不同看法。

```gherkin
Scenario: 新建立的策略腳本不再帶著取數計畫
  Given 一個名為「二十根均線」、指標值種類為一串數字的策略腳本內容
  When 建立這支策略腳本
  Then 建立成功
  And 讀回這支策略腳本時只有策略腳本名稱、指標算式、指標值種類、建立時間與最後修改時間
  And 回覆中不出現彙總刻度，也不出現計算根數

Scenario: 既有策略腳本的算法原封不動
  Given 一支既有策略腳本，原本記著一小時的彙總刻度與 20 的計算根數
  When 讀取這支策略腳本
  Then 它的策略腳本名稱、指標算式、指標值種類與建立時間與先前完全相同
  And 回覆中不出現彙總刻度，也不出現計算根數

Scenario: 附上取數計畫也不會被記住
  Given 一份建立策略腳本的內容，其中仍舊附上一小時的彙總刻度與 20 的計算根數
  When 建立這支策略腳本
  Then 建立成功
  And 讀回這支策略腳本時不出現彙總刻度，也不出現計算根數

Scenario: 修改策略腳本時沒有取數計畫可以一起改
  Given 一支既有策略腳本
  When 把它的策略腳本名稱與指標算式改成新的值
  Then 修改成功
  And 最後修改時間已更新
  And 讀回這支策略腳本時不出現彙總刻度，也不出現計算根數

Scenario: 名稱為空白仍然被拒絕
  Given 一份策略腳本名稱為空白的建立內容
  When 建立這支策略腳本
  Then 拒絕建立，說明必須給策略腳本取一個名稱

Scenario: 名稱重複仍然被拒絕
  Given 已有一支名為「二十根均線」的策略腳本
  When 再建立一支名為「二十根均線」的策略腳本
  Then 拒絕建立，說明該名稱已被使用
  And 既有那一支完全不受影響
```
...
**As a** 拿策略腳本去算的人，**I want** 在執行計算時指定要多粗的 K 線、要幾根、算到哪個時間為止，
...
**As a** 拿策略腳本去算的人，**I want** 系統只把已經走完的刻度區間交給算式，
...
**As a** 拿策略腳本去算的人，**I want** 資料不足時被明確拒絕，
...
**As a** 寫策略腳本的人，**I want** 由算式自己決定它需要幾根才算得準，
**so that** 系統不必替我猜，我也不必再為此在策略腳本上多存一個欄位。
...
- **策略腳本記住三樣**：策略腳本名稱、指標算式、指標值種類。彙總刻度與計算根數不再屬於策略腳本。
...
- **最新那一格可能還在等資料。** 「走完」是照時鐘算的，不是照「存到哪裡」算的，
  而抓取是在那五分鐘過完之後才寫入。因此以「現在」為截止時間問到的最新那一格，
  可能少了最後一兩根還沒寫進來的 K 線，過幾分鐘再問同一個問題會得到不同的值。
  **不以「那一格要裝滿才算數」來解**——那條規則分不出「市場沒有成交」與「資料還沒到」，
  會讓成交稀疏的標的永遠拿不到最新那一格。需要穩定答案的呼叫端指定一個過去的截止時間。
- 交易標的一根 K 線都沒有 → 湊不滿，整次拒絕。
- 走完的格子夠多、但中間有空洞 → 空洞那格不產出，其餘湊得滿就算得成。
- 計算截止時間早於系統最早的一根 K 線 → 湊不滿，整次拒絕。
- 算式一個指標名稱都沒放進結果 → 仍是一次成功的計算，起始時間照樣回覆。
- 既有策略腳本帶著彙總刻度與計算根數 → 那兩樣直接消失，其餘內容原封不動。
- 建立或修改策略腳本時仍舊附上那兩樣 → 忽略，不留下、也不因此拒絕。
...
- **Performance:** 一次計算屬於即時讀取，一次互動一次回覆。
  ...
  由較粗的刻度回看很長一段時，讀取量會顯著大於彙總後的根數。

  **要講清楚的是：單次查詢筆數上限數的是彙總後的根數，因此它不再約束這條路徑的讀取量。**
  一天的刻度 × 1000 根會讀進 288,288 列原始 K 線。這是刻度變粗的必然成本，
  本切片**刻意不另設第二道上限**——加一道就等於憑空多一種拒絕，而目前的資料量
  （啟動回補上限 24 小時）連一天的刻度都湊不滿一根。
  **該回頭處理的訊號**：出現「回看很長一段」的需求，或單次計算的讀取時間變得明顯。
- **Security:** 不變。指標算式仍只能做純運算，不得取用 K 線以外的資料、當下時間或任何隨機值；
  執行逾時即中止。
- **Compatibility:** 這是一次**破壞性變更**：策略腳本的形狀少了兩樣、計算的請求與回覆都多了東西。
...
- **Known Risks:**
  ...
  - **既有策略腳本的那兩樣直接消失**：無法復原。可接受——它們從未生效，消失的是沒有人讀的願望。
