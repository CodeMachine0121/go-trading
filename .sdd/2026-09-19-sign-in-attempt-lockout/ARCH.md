...
**解除時刻另外再以 `lockedUntil` 欄位單獨回一次**（RFC3339、UTC）。
句子是寫給人看的，而呼叫端要把那個時刻換算成讀者自己的時區——
從一句寫給人看的話裡正則挖出時間，會在下一個人改寫那句話的那天靜靜地挖不到。
這是這個功能裡唯一一個回應多一格的地方，理由就是它：**同一個事實，一份給人讀、一份給程式讀。**
...
- **讀後寫的競態——原本判斷錯了，已修正。**
  初版寫成「偶爾多放過一次嘗試，接受」。那個判斷只在「兩個人同時打錯字」的情況下成立。
  實際上攻擊者可以自己決定並行度：同時對一個位址送 100 個請求，
  它們全部讀到同一個數字、全部寫回同一個數字，於是**100 次猜測只花掉一次**，
  而門檻永遠到不了——那正是這道鎖要擋的「一台不會累的機器」。
  實測證實：拿掉守衛後，100 次並行嘗試最終只算到 **5**。

  **修法沿用這個 repo 既有的慣例**（`session_repository.go` 換發那一段）：
  寫入多一個 `WHERE failed_sign_in_count = <讀到的那個數字>` 的守衛，
  沒套用到就回哨兵錯誤 `ErrSignInLockoutStateStale`，由 service 重讀後再數一次。
  規則仍然住在 domain，儲存層只多一個守衛欄位，沒有任何業務計算被搬進 SQL。

  重試次數固定為 3 且**不是設定**：每一輪失敗都代表別人的數字有落地，
  所以門檻是 3 時不可能三輪都落空。重試用完**不靜默放過**——
  數不到的嘗試就是沒有人知道的嘗試，整次登入失敗。
