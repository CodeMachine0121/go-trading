...
Scenario: 停機超過今天,更早的補不回來
  Given 系統停機三天
  And 台灣期貨的行情來源只答得出今天兩段還握著的行情
  When 系統啟動
  Then 補回今天兩段還握著的每一根
  And 更早的那幾天補不回來,而且不算失敗
```
...
- **Core Business Rules:**
  ...
  - **台灣期貨的即時跟盤沿用「有跟盤上限的市場走固定名單」這條既有規則**，
    不因為市場是期貨而另立一套。
  - **即時跟盤時，有兩段交易時間的市場要向來源同時要兩段。**
    來源把兩段分開發布、而且只推送被要求的那一段，
    所以只要一段的話**夜裡會完全安靜**——而安靜看起來就跟「市場沒事發生」一模一樣，
    正是這個切片要避免的那種沉默。哪一段正在跑是系統自己的知識，
    不在跟盤這一側重算：兩段都要，沒在跑的那一段自然不說話。
...
- **Known Risks:**
  ...
  - **行情來源對最近到期那一口的說法若不穩定**（在兩口之間來回），
    系統會跟著來回，而那條線會出現兩次跳空。
  - **這個來源的期貨只有今天的行情，沒有更早的。** 因此停機超過一天之後，
    更早的那幾天補不回來——沿用既有規則「來源手上沒有就回幾根存幾根，不算失敗」。
    長時間停機留下的缺口是真的缺口，圖上會看得見。
