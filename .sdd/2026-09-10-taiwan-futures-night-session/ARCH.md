...
- **實作後才確定的兩件事（下一個人請先讀）:**
  - **這個來源的期貨只有日內行情，沒有歷史行情——這是查證過的，不是查不到。**
    該來源的期貨行情文件底下只有「日內行情」與即時串流兩塊，沒有 historical 那一組；
    它確實有歷史行情位址，但那是**股票**那一組的（一年內），不涵蓋期貨。
    因此**啟動回補只補得回今天兩段還握著的部分**，更早的補不回來——沿用既有規則
    「來源手上沒有就回幾根存幾根，不算失敗」。長時間停機後的缺口是真的缺口。
  - **真要有期貨歷史，得換一個來源**：交易所自己每日公布可下載的歷史檔（含盤後時段），
    免費但隔日才有。那會是另一個 proxy 掛進同一張路由表，
    `IMarketDataProxy` 一個字都不用改——但它是另一個切片，不在這一版。
