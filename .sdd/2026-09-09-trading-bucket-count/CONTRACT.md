...
第一版用**逐格走訪加一個集合**收格子起點。Review 用實際量測抓到三個 HIGH，
其中兩個是同一個原因：

- **沒有東西限制呼叫端問多長的一段。** 一個世紀在一分鐘刻度上是 8,875,710 格，
  走訪它花掉 **771 MiB、1.47 秒**——而且發生在「區間過大」那句拒絕說出口**之前**。
  指標計算那一側更糟（1286 MiB），因為它在自己的上限檢查之前就先問了一次。
- **把「格數等於零」讀成「沒有交易」**。格數會取整，所以全天候市場的三十秒被讀成零格，
  於是一個永不收盤的市場被回報成「沒有交易」——而通用語地圖與市場那份文件
  都寫著那不可能發生。這是這個切片自己造成的回歸。
- 順帶：零長度的刻度會讓那個走訪**永遠不結束**（`time.Truncate` 對非正數原樣回傳），
  比它取代的那個除法 panic 更糟。

改成**逐段以算術數格子**，一次解掉三個：O(1) 記憶體、沒有迴圈可以卡住，
而「有沒有交易」改成 `HoldsTrading` 自己回答。
**每一個格數一字未變**——算術與走訪對今天的資料答案相同，17 個 package 的斷言全數未改即通過。

同時刪掉了第一版為「一天兩段交易時段」預留的那一行扣減：它今天**沒有任何輸入走得到**
（一天一段、最粗一天，兩段不可能落在同一格），因此是死碼。
警告搬到 `eachTradingDaySession`——那才是加第二段交易時段時真正會被改的地方。

四個新變異全部被抓到：把「有沒有交易」改回讀格數、讓全天候市場永遠不開盤、
收盤鐘聲當成可交易、逐段的格數丟掉包含的那一端。

**一個沒修的 HIGH（另開切片）：全天候市場那條分支仍然在除。** 見 `ARCH.md` §8——
它的行為在本切片前後一字不變，而修它會移動這條路上每一個格數與使用者看到的每一根 K 線。
