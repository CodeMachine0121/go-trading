...
- **Problem Statement**
  使用者寫得出一段指標算式，也看得到它在最新那一批 K 線上算出什麼數字，但系統答不出他真正想問的那一句：「如果我從一年前就照這支策略腳本操作，現在手上會剩多少錢？」指標值只說「現在是多少」，不說「一路走下來會怎樣」。中間差的是**時間**——策略腳本要在每一根 K 線上各判斷一次，而不是只在最後一根上判斷一次。

- **Expected Outcome**
  使用者選一段時間、一筆初始資金、一種倉位大小模式，系統把那段歷史**從頭重演一遍**，逐根 K 線執行同一支算式、照它說的話模擬進出場，最後交出**這段期間的成績單**：總報酬率、最大回撤、勝率、開倉次數，加上每一筆已平倉交易與一條資金曲線。「這支策略腳本值不值得信」從此是一段可以重算、可以拿出來看的紀錄，而不是感覺。
...
- **Primary Role**：策略腳本作者（本系統唯一的使用者）。他自己寫指標算式、自己調參數、自己判斷一支策略腳本值不值得留下來。
- **Usage Context**：在指標計算的工作區裡，算式剛剛改過、參數剛剛調過，想立刻知道「這樣調到底比剛才好還是壞」。他會在敲定一支策略腳本之前重跑幾十次，所以每一次重演都是**即問即算、算完就走**。
...
**As a** 策略腳本作者，**I want** 在既有的指標算式裡多放一個叫「信號」的指標名稱，**so that** 同一段算式既能算指標數字，也能在重演時說出每一棒要買、要賣、還是不動。
...
Scenario: 信號算不出一個有限的數字
  Given 某一棒算出來的信號不是有限數字（例如零除以零，或除以零得到的無限大）
  When 重演讀這一棒
...
**As a** 策略腳本作者，**I want** 重演在任何時刻最多只持有一個倉位、聽到反向信號就在同一棒反手，**so that** 模擬出來的進出場與我腦中「一次只押一注」的操作方式一致。
...
Scenario: 這一棒的收盤價不大於零
  Given 手上沒有倉位
  And 這一棒的收盤價是 0
  When 這一棒說買入
  Then 這次開倉被跳過，手上仍然沒有倉位
  And 重演不中斷、開倉次數不增加——價格是零的市場沒有東西可以押
```
...
**As a** 策略腳本作者，**I want** 選擇每次開倉要押全部資金、押某個百分比、還是押固定金額，**so that** 重演出來的資金曲線反映我實際打算怎麼下注。
...
**As a** 策略腳本作者，**I want** 資金曲線一根 K 線一個點、且把還沒平掉的倉位一起估進去，**so that** 我看得出資金一路走下來的形狀，而不只是最後那個數字。
...
**As a** 策略腳本作者，**I want** 拿到一張把賺賠、風險與命中率講清楚的成績單，加上每一筆已平倉交易，**so that** 我判斷這支策略腳本值不值得留下來。
...
**As a** 策略腳本作者，**I want** 一次重演明確說出看哪個市場、用多粗的 K 線、從哪裡到哪裡、用哪一支算式與它這次的參數值、以及本金與押注方式，**so that** 同樣的問題問兩次會得到同樣的答案。
...
**As a** 策略腳本作者，**I want** 重演在做不到的時候明講是哪一件事不成立，**so that** 我知道要去改哪一格，而不是對著一個空白畫面猜。
...
- **BR-1 信號的讀法**：從一棒的指標結果中取名為「信號」的那個值。**大於零讀作買入，小於零讀作賣出，等於零讀作持平**；沒有這個名字、或算出來的不是一個**有限**數字（NaN 或無限大），一律讀作**持平**。這條讀法讓既有算式一個字都不必改。
...
- **BR-6b 押不下去的兩種情形**：可用資金不夠（見 BR-7），或**該棒的收盤價不大於零**。兩者都是「這一次開倉沒有發生」，重演照常往下走。
...
- **為什麼不留存**：重演是探索用的工具。敲定一支策略腳本之前會跑幾十次，存下來的那幾十份沒有人會回頭看，卻要有人負責清掉。
...
| 待定事項 | 定案 | 理由 |
| :--- | :--- | :--- |
| 信號說了 1／-1／0 以外的數字 | **看正負號**：大於零買入、小於零賣出、等於零持平；不是有限數字則持平 | 信號本來就是「方向」。看正負號讓每一個數字都有明確意義，不會有「合法但無解」的值，也與 1／-1／0 的既有寫法完全相容。逐棒重演途中因為某一棒算出 0.5 就整次拒絕，代價與這條規則的收益不成比例 |
