...
| `AssistantRevisionService` | Domain Service | 待確認修改的整個生命週期，一個業務動作一個方法：`Revise`（檢查 → 直接生效或留下待確認）、`ConfirmPendingRevision`（找出 → 檢查被改過 → 以條件式狀態轉換搶下 → 寫入 → 被拒則退回待確認）、`RejectPendingRevision`、`RecordCreatedSubject`；依種類挑 applier | `IAssistantPendingRevisionRepository`、`IAssistantCreatedSubjectRepository`、`[]IAssistantRevisionApplier`、`IClockProxy` | US-03 |
...
| `AssistantRevisionApplication` | Application | 把上面四個動作交給 service；只多一條「記下建立失敗不讓建立失敗」 | `AssistantRevisionService` | US-03 |
...
| `StrategyScriptReferencesDto` / `RewriteTargetDto` / `AssistantPendingRevisionDto` / `AssistantRevisionProposalDto` | DTO | 跨邊界的資料形狀 | — | — |
...
flowchart TD
    ...
    RS --> AP{{IAssistantRevisionApplier}}
...
**Revise**（全在 service 內）：applier.Inspect（找不到、內容不合法 → 原因交回助手、不留提議）→ 本段對話建立且沒有機器人引用 → applier.Apply；
否則存待確認並回報 pending。

**Confirm**（全在 service 內）：找出（別人的＝找不到、已處理）→ applier.Inspect（目標現況）→ 被改過 → 作廢錯誤 →
條件式 pending→confirmed（搶輸 → 已處理）→ applier.Apply → 失敗則退回 pending 並回原因。呼叫端不必知道任何步驟順序。

建構順序無循環：applier（需既有 application）→ `AssistantRevisionService` → `AssistantRevisionApplication` → 助手查詢 → `AssistantConversationService`。與 `IAssistantQuery` 同一種做法：domain 介面、application 層實作、domain service 呼叫。
...
| US-01 助手的修改在確認時一樣被擋 | `AssistantRevisionService.ConfirmPendingRevision`（Apply 失敗 → 退回 pending） |
...
| US-03 只留下待確認修改／交易策略一樣／提出時不合法不留 | `AssistantRevisionService.Revise` + applier.Inspect |
| US-03 確認後生效／拒絕 | `AssistantRevisionService.ConfirmPendingRevision` / `RejectPendingRevision` |
| US-03 處理過的不能再處理（確認、拒絕） | `AssistantPendingRevisionDomain` + 條件式狀態轉換 |
| US-03 被改過不能確認／同一次回答兩筆 | `ConfirmPendingRevision` + `AssistantPendingRevisionDomain.RequireUnchangedSince` |
| US-03 確認時被一般規則擋下 | `ConfirmPendingRevision`（Apply 失敗 → 退回 pending） |
