...
Scenario: 執行中的機器人使用它時拒絕改寫
  ...
  When 我改寫 A
  Then 改寫被拒絕，並說出「這幾台機器人正在用它跑：X，請先停止它們」
  And A 一個字都不變
...
Scenario: 只要有一台執行中就擋，只點名執行中的那幾台
  Given 我有兩台機器人都用到策略腳本 A，執行中的 X 與已停止的 Y
  When 我改寫 A
  Then 改寫被拒絕，並說出「這幾台機器人正在用它跑：X，請先停止它們」
  And 那句話不提到 Y
...
Scenario: 助手的修改在確認時一樣被擋
  ...
  And 一筆把 A 改寫的待確認修改
  When 我確認那筆修改
  Then 確認被拒絕，並說出「這幾台機器人正在用它跑：X，請先停止它們」
  And 那筆修改維持待確認
```
...
Scenario: 系統自己說的原因照舊交出
  Given 策略腳本 B 屬於別人
  And 這一段時間 B 的交易標的沒有交易
  When 助手用 B 算指標
  Then 助手拿到的原因是「要看的這一段時間裡沒有交易」那一句

Scenario: 別人的算式算太久也不交出細節
  Given 策略腳本 B 屬於別人
  And B 算太久被中止
  When 助手用 B 算指標
  Then 助手拿到「這支策略腳本不是你的，它執行失敗，不提供細節」
...
Scenario: 交易策略裡只要有一個別人的來源，重演時任何算式失敗都不交出細節
  Given 我的交易策略 S 有兩個信號來源，一個指名我自己的策略腳本 A、一個指名別人的策略腳本 B
  And 重演時 A 以一段文字失敗
  When 助手重演 S
  Then 助手拿到「這支策略腳本不是你的，它執行失敗，不提供細節」
...
- **別人的算式失敗**：以「提問者是不是這支策略腳本的擁有者」判斷，**不以有沒有發佈判斷**。
  **出自算式隔間的一切失敗**（含算太久被中止——那一句與作者寫的字從同一個出口出來，分不開）→ 固定一句話；
  在算式開跑之前系統自己說的原因（算式隔間忙碌、區間落在收盤時間、根數不夠、找不到、刻度不一致）→ 原句。
  一次重演裡**只要有一個信號來源是別人的**，那次重演的算式失敗一律視為別人的——說不出是哪一個來源失敗的。
  只作用在交給助手的那一份。
- **待確認修改的內容是完整的改寫後樣子**，與一般改寫送進去的內容相同，不是差異。
- **「提出之後被改過」** 以目標的最後修改時刻判斷：與提出時記下的不同即視為被改過。
- **助手本段對話建立的**：該段對話裡任何一次回答中，助手的建立成功留下的那一支／那一份。
  「沒有任何機器人引用」**不分是誰的機器人**：你發佈的腳本被別人的機器人用到時，助手改它同樣要你確認——
  寧可多問一次，也不讓別人的機器人在沒人點頭的情況下換一套規則。
- **待確認修改裡只能有那項能力收的欄位**：多一個它不認得的欄位即整次拒絕並把原因交回助手，
  否則使用者看到的內容與真正寫進去的會不一樣。
- **提出之前先做一次一般檢查**：找不到、別人的、內容不合法的，照舊把原因交給助手、不留提議；
  執行中機器人的阻擋**不在提出時擋**——使用者可以先停機器人再確認。

### Edge Cases

- **確認時系統出錯**：該筆維持待確認，使用者可再按一次——**連按下確認的那一方中途離開或逾時也一樣**。
...
- **Compatibility:** 既有對話讀回時照舊；沒有待確認修改的一次回答不帶這一項。
  待確認修改記在一次回答**最後一則訊息**上：寫完的在回答那一則，進行中或失敗的在提問那一則。
