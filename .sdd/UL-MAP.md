...
| 指標結果 | *(尚未實作)* | 指標結果 | 一次計算的產出：**一組「指標名稱 → 指標數值」**。所有指標共用這一種形狀，放幾個值由算式決定 | Confirmed |
| 指標名稱 | *(尚未實作)* | 指標名稱 | 指標結果中每個數值的名字，由算式作者自行決定。同一組結果中重複時由後放的覆蓋先放的 | Confirmed |
| 指標數值 | *(尚未實作)* | 指標數值 | 指標結果中對應某個指標名稱的數字。為一般數字，不要求金額等級的精確度 | Confirmed |
...
| 執行指標計算 | *(尚未實作)* | 使用者提供交易標的、計算根數與一段指標算式 | 取出該交易標的**最新一根以外**、由早到晚的指定根數 K 線交給算式執行，回傳指標結果。結果不留存 | 可用根數不足、計算根數不大於零或超過上限、算式無法解讀、算式執行失敗、算式試圖取用不該取用的東西，一律拒絕整次計算並說明原因，且不回傳任何部分結果 |
