...
| 指標算式 | *(尚未實作)* | 指標算式 | 使用者自行撰寫的一段計算規則，收下一批 K 線並算出指標。只能做純運算：不得取用 K 線以外的資料、當下時間或任何隨機值 | Confirmed |
...
| 算式可用運算 | 四則運算、比較、重複計算、開根號、對數、次方、絕對值、極大值、極小值 | 算式運算範圍 | 算式只能用到這些；取用檔案、網路、當下時間或隨機值一律拒絕 |
| 算式執行時間上限 | 無 | 算式執行時間上限 | 刻意不設。算式若永遠算不完，該次計算會一直等下去 |
