...
| 單輪取回根數 | *(尚未實作)* | 單輪取回根數 | 每一輪針對單一交易標的取回幾根已收完的 K 線。預設**五根**，可調整。取多於一根是為了讓行情來源事後修正過的數字自動覆蓋進來 | Confirmed |
...
| 自動抓取 K 線 | *(尚未實作)* | 每隔五分鐘自動觸發一輪 | 替觀察清單上的每個交易標的取回最近數根**已收完**的 K 線並存入；既有的以新資料覆蓋，不產生重複 | 進行中 K 線不存入；各交易標的彼此獨立，一個取不到不影響其他；違反 K 線規則者**逐根跳過**並留下紀錄，同批其餘照常存入 |
