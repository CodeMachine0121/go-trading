...
- **純運算規則**：算式只能用基本四則運算、比較、重複計算，以及常用的數學運算
  （開根號、對數、次方、絕對值、極大值、極小值）。不得取用 K 線以外的資料、
  當下時間或任何隨機值——同一批 K 線跑兩次，結果必須完全相同。
- **失敗規則**：算式無法解讀、執行失敗、或試圖取用不該取用的東西，一律拒絕整次計算並說明
  原因，且**不回傳任何部分結果**。
...
- **K 線在區間內不連續（中間缺根）** —— 有幾根算幾根，系統不補洞、不告警；
  只要可用根數足夠就照常執行。
- **算式永遠算不完** —— 本階段**不設執行時間上限**，該次計算會一直等下去。
  這是明確接受的取捨，記於第 7 節。
...
- **Performance:** 本階段不設定回應時間目標，也**不設算式的執行時間上限**。
  唯一的規模限制是單次可用的最大根數（與單次查詢筆數上限共用設定）。
...
- **沒有執行時間上限**（使用者明確選擇）—— 一段永遠算不完的算式會讓該次計算一直等下去，
  沒有自動恢復的手段。目前唯一的緩衝是單次最大根數。
  **該重看的訊號**：出現第一次算式卡住、必須手動重啟服務的時候。
