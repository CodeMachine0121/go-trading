...
| T7 | `Value` 的長度 | ≥ 26，且每次一樣長 |

> T7 原本寫「≥ 43」，那是先假設了實作會自己取 32 位元組再編碼。改用標準函式庫
> 專為此用途提供的亂數字串產生器之後，長度是 26（至少 128 位元的亂數，函式庫明文保證
> 「足以擋住暴力猜測、碰撞機率微乎其微」，且承諾未來會視需要加長）。
> **這是把先前寫錯的假設改正，不是為了遷就寫出來的程式碼調低標準**——
> 標準本來就該是「猜不到」，而不是某一個特定的位元組數。
