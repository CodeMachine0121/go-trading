...
- **Out of Scope:**
  - 替既有的行情端點（K 線、指標計算、策略腳本、助手、即時跟盤）裝上門。理由不變：
...
- **Edge Cases:**
  - 兩個續用請求同時帶著同一份憑證到達 → **兩邊都會讀到一段還沒作廢的登入階段**
    （讀取與寫入是兩個時刻），所以「只能用一次」這件事**不能靠讀取成立，只能靠寫入成立**：
    作廢舊的那一步帶著「它還沒被作廢」這個條件，慢的那一個因此一列都動不到，
    於是被判定盜用並撤掉整條鏈。**這是可接受的誤判**：代價是重登一次，
    而放行的代價是一份用過的憑證換出了第二段有效的登入階段，盜用偵測形同虛設。
  - **同理，續用與登出同時發生時，登出不會被續用悄悄還原。** 一條剛被撤掉的鏈
    已經沒有還沒作廢的那一列可以給換發對上，所以換發會被拒絕——
    否則使用者按了登出、拿到 204，那台裝置卻還握著一份能用三十天的續用憑證。
