...
Oracle: Acceptance Criteria (22 scenarios) + Core Business Rules (7) + Non-Functional (2) = 31 clauses
...
| AC-17 | 計算根數超過單次上限 | 照原樣整次拒絕，並提示縮短觀察區間或改用粗一點的彙總刻度 | `indicator_calculation_errors.go:26`、`indicator_calculation_controller.go:60` | `indicator_calculation_controller_test.go:259` | asserts-oracle（就程式實際產出而言） | **diverges from the literal clause**：訊息只說「用到 N 根，超過上限 M 根」，並以 `field: startTime` 指出該改哪一格；**沒有任何一句話提到「或改用粗一點的彙總刻度」** | 🟠 mis-asserted |
| AC-18 | 觀察區間涵蓋三個平日、其中一天休市 | 要看 162 格（不預先扣除）；回報需要 162、實際採用 108 | `market_domain.go:94`（只看星期與時段） | — | no-test（沒有任何測試造出「平日但整天沒有 K 線」） | produces-oracle（推算不查假日名單；落差由既有兩個根數回報） | 🟡 partial |
...
| BR-5 | 休市日不預先扣除 | 同 AC-18 | `market_domain.go:328`（`eachTradingDaySession` 不查假日名單） | — | no-test | produces-oracle | 🟡 partial |
...
|------|-------------|---------|
| `observation_window_domain.go:52` | 起點**等於**終點（長度為零的區間）也被拒絕。PRD 只寫了「起點晚於終點」，等於的那一個邊界沒有條款 | undocumented — 建議補一條邊界條款（它取代了舊的「計算根數必須大於零」，讓「要求零格」仍然被擋下） |
| `observation_window_domain.go:47` | 未指定起點即拒絕（「必須指定要看哪一段的起點」）。PRD 的 US-06 沒有這一列 | undocumented — 建議補條款；ARCH 的 Open Decisions 已記下這個決定，但沒有回填 PRD |
| `indicator_calculation_controller.go:63` | 「要太多」的回應把 `field` 從 `candleCount` 改為 `startTime` | undocumented — 對外形狀改變，PRD 未描述（與 AC-17 相關） |

## Summary

- Conforms: 26/31 clauses ✅ (84%)
- Violations: 無
- Mis-asserted: AC-17（**修的是條款不是程式**：PRD 那句「並提示縮短觀察區間或改用粗一點的彙總刻度」沿用自 BRIEF，而這條路徑 PRD 自己寫明「照原樣不改」——既有訊息從來沒說過那兩條出路。建議把條款改寫成程式實際說的話，或另開切片把出路寫進訊息）
- Partial: AC-18、BR-5（休市日落差無測試）、NFR-1、NFR-2（結構性條款，以閱讀驗證）
- Gaps: 無
- Unclear: 無
- Orphans: 3（皆為建議回填 PRD 的邊界與對外形狀，非越界實作）
