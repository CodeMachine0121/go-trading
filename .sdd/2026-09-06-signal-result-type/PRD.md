...
- **Out of Scope:**
  ...
  - 一次執行產出多個信號。
  - 把信號種類的算式存成策略腳本後的額外驗證或預跑——沿用策略腳本「不執行也不檢查算式」。
...
- **Primary Role(s):** 策略腳本研究者（專案作者本人）——同時是算式的作者與結果的讀者。
...
**As a** 策略腳本研究者, **I want** 在送出計算時把指標值種類宣告成「信號」,
...
**As a** 策略腳本研究者, **I want** 一個信號只可能是買入、賣出、持有三者之一,
...
**As a** 策略腳本研究者, **I want** 宣告信號種類時算式每次執行都要給出一個明確方向,
...
**As a** 策略腳本研究者, **I want** 回測一律以信號種類執行算式、不再看數字的正負號,
...
### US-05 — 信號種類在單純算指標與策略腳本也能用 [priority: P1]

**As a** 策略腳本研究者, **I want** 單純算一次指標時能宣告信號種類、建立策略腳本時能把種類設成信號,
**so that** 我可以先預覽一支策略腳本在最新資料上的信號，也可以把「會出信號的策略腳本」存起來。
...
Scenario: 建立策略腳本時把種類設成信號
  Given 使用者建立一支策略腳本，指標值種類設成「信號」
  When 使用者送出建立
  Then 策略腳本正常建立
  And 不會因為「種類不在四種之內」被拒

Scenario: 修改策略腳本時把種類改成信號
  Given 一支已存在的策略腳本，原本的指標值種類是「一個數字」
...
**As a** 策略腳本研究者, **I want** 取 K 線、根數上限、算式時間上限、回測的逐棒規則在信號種類下都一樣,
...
- **BR-9 信號種類到處可用**：單純算指標可宣告信號種類（回傳一個沒有名稱的信號）；建立與修改策略腳本時指標值種類可以是信號。
...
- **已存的舊策略腳本其算式其實在用數字表達信號**：這一版不主動掃描，等使用者拿去回測才會被擋下並被告知要改寫。
...
3. 策略腳本已存成舊的種類、其算式其實在用數字表達信號時，是否要主動標記為「需改寫」，還是等使用者自己拿去回測才發現（本 PRD 傾向後者）。
