...
| 建議 | 為什麼現在不做 | 該回頭處理的訊號 |
| :--- | :--- | :--- |
| 把 `KCandleFollowService` 再拆成「註冊表」與「單一市場的跟盤迴圈」兩個物件 | 拆完之後，兩者之間要傳的正是現在的三個設定值與 domain，而「開一份／收一份」的判斷會橫跨兩個物件——反而讓最重要的那條規則沒有單一歸屬 | 這個檔案再長出第三種責任時（例如要記錄跟盤統計、或支援多種行情），屆時拆點會自己浮現 |
| `leave` 中「這位觀看者已經不在了」那個防衛分支沒有測試覆蓋 | 想不出能構造它的情境（每位觀看者的離開只會發生一次），但刪掉它的失敗模式是 `close(nil)` panic。**在併發路徑上，我不拿一個防衛換一個 panic** | 若日後 `leave` 的呼叫點超過一處，這個分支就會變成可達，屆時補測試 |
| 推播事件的欄位命名尚未與前端一起定案 | 前端切片還沒開始；現在定等於猜 | 前端開始接這條路徑時 |
