...
| AC-13 | 只差前後空白的名稱視為重複 | 拒絕——前後空白不計入，兩者是同一個名稱 | `dom:44`（去空白）+ `ent:20`（索引） | `Trepo:49`、`Tdom:86` | asserts-oracle | produces-oracle | ✅ |
...
| AC-13 | 🟡 partial | 去空白與索引各自有測試，合起來沒有 | 新增 `Trepo:49`：去空白後同名即衝突 | 唯一索引降為一般索引 → 紅 |
