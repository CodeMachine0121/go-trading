...
Oracle: Acceptance Criteria (36 clauses — 23 scenarios, 10 business rules, 3 non-functional; Compatibility is `N/A` and skipped)
...
| AC-07 | 取回的資料部分已有、部分尚無 | 3 根覆蓋、2 根新增、沒有任何起始時間出現兩根 | `k_candle_ingestion_service.go:148`（逐根 `Save`）、`k_candle_repository.go:32` | `k_candle_repository_test.go:319` | asserts-oracle | produces-oracle | ✅ conforms |
...
| AC-14 | 全部交易標的都取不到資料時系統仍繼續運行 | 每個標的各留下一筆失敗紀錄；系統不停止，五分鐘後仍會跑下一輪 | `k_candle_ingestion_service.go:102`、`k_candle_ingestion_job.go:70` | `k_candle_ingestion_service_test.go:243`、`k_candle_ingestion_job_records_test.go:142` | asserts-oracle | produces-oracle | ✅ conforms |
| AC-15 | 上一輪失敗漏掉的 K 線在下一輪自動補回 | 下一輪之後，上一輪漏掉的 09:00 K 線已存入，且無人工介入 | `k_candle_ingestion_domain.go:60`（窗口寬度＝單輪取回根數） | `k_candle_ingestion_service_test.go:472` | asserts-oracle | produces-oracle | ✅ conforms |
...
| BR-02 | 抓取間隔規則：固定五分鐘一輪，不可調整；停用走總開關或空清單 | 間隔恆為五分鐘，且沒有任何設定能改它 | `k_candle_ingestion_job.go:16`（常數）、`dependencies.go:82`；`application_config.go` 無對應設定 | `k_candle_ingestion_job_test.go:130` | asserts-oracle | produces-oracle | ✅ conforms |
...
| BR-07 | 並行規則：同一輪內所有交易標的同時進行，彼此獨立 | 多個標的同時開始，一個的結果不影響另一個 | `k_candle_ingestion_service.go:109`（`sync.WaitGroup`，每標的一個 goroutine） | `k_candle_ingestion_service_test.go:223`（只證獨立） | shallow | produces-oracle | 🟡 partial |
...
| NFR-02 | Security：沒有身分驗證與權限控管；不對外開放任何操作進入點，觀察清單無法從外部改變 | 本功能沒有新增任何路由；清單只能由啟動設定決定 | `dependencies.go`（`registerRoutes` 未更動，本次僅新增 `backgroundJobsFor`）、`k_candle_ingestion_job.go:38` | `k_candle_ingestion_job_test.go:108`、`routes_test.go:14` | asserts-oracle | produces-oracle | ✅ conforms |
| NFR-04 | Analytics：不埋任何使用行為追蹤；僅在失敗與跳過時保留紀錄 | 正常輪次不留任何紀錄；只有失敗與跳過會寫 | `k_candle_ingestion_job.go:85`-`:106` | `k_candle_ingestion_job_records_test.go:103`、`:116`、`:130`、`:166`（含「無事則不寫」） | asserts-oracle | produces-oracle | ✅ conforms |
...
|------|-------------|---------|
| `internal/job/k_candle_ingestion_job.go:55` | `Stop()`：關閉工作、結束定時迴圈。PRD 沒有任何條文提到停止背景工作這件事（`AC-23` 講的是啟動時就不組裝）。ARCH 把 graceful shutdown 列為未來事項 | undocumented（**非** Out of Scope 違規） |
| `internal/infrastructure/marketdata/binance_market_data_proxy.go:111` | 丟棄落在窗口之外的 K 線，並因此停止繼續索取。實作階段由測試逼出的防線，PRD 與 ARCH 都沒有對應條文 | undocumented（**非** Out of Scope 違規） |
| `internal/domain/service/k_candle_ingestion_service.go:48`、`:65` | 單輪取回根數不大於零時，兩個用例都拒絕執行並回傳錯誤。PRD 沒有這條 | undocumented（**非** Out of Scope 違規） |
...
- Conforms: 34/36 clauses ✅ (94%)
- Violations: 無 🔴 —— 沒有任何一條的程式碼產出與 spec 相反
- Mis-asserted: 無 🟠
- Partial: `BR-07` 🟡
- Gaps: 無 ❌
- Unclear: `NFR-01` ❔
- Orphans: 3 ⚠️（皆為未記載行為，非界外違規）

### 仍未關閉的兩條

- **`BR-07` 並行規則 🟡** —— 現有測試證明各交易標的**彼此獨立**，但沒有證明它們**同時**開始。
  這一條做得到：讓行情來源在收到第一個標的時擋住、等到第二個也進來才放行——
  若實作是逐一執行就會卡死逾時。要我補就說一聲。
- **`NFR-01` 效能 ❔** —— 「一輪必須在五分鐘內完成」取決於觀察清單長度與來源的實際回應時間，
  靜態判不了，也不該用單元測試假裝。建議實際跑起來之後量。

### 未處理的 3 個 orphan

`Stop()`、proxy 丟棄窗口外資料、根數不合法時拒絕執行——都還沒進 PRD。
前兩者外部觀察得到，仍建議補成條文。
