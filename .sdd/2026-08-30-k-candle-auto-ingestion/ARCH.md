...
- **Patterns applied & why:**
  ...
  - **刻意不抽「來源種類」的策略腳本階層**：Binance 與其他交易所是同一個介面的不同**實作**，
    不是需要另一層策略腳本選擇器的東西。
...
**`KCandleIngestionJob.Stop()` 目前沒有任何正式程式碼會呼叫它。**
它存在的實際理由是讓工作在測試結束時停下來——否則背景的 goroutine 會在測試替身
收攤之後繼續呼叫它們。這是一個真實的生命週期能力，但**行程目前就是直接結束**，
沒有任何關閉流程。

因此本切片**刻意不為它立下 PRD 條文**：為一個不會發生的行為寫驗收條件，
會讓契約說謊。要讓它成為可依賴的行為，應另開一個處理整個服務關閉流程的切片
（訊號攔截、HTTP 伺服器收尾、背景工作停止三者一起），那才是它該待的地方。
