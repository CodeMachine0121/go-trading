...
往後推到「已存最新那一根之後」；這一個不推，因為整段重抓正是它存在的理由。

### `KCandleIngestionService`

多一個公開用例 `SyncHistoryFor`。它與 `RunBackfillFor` **互不呼叫**（同一個 service 的
公開用例互不呼叫），但兩者共用同一段私有流程：
...
| US-01 整段重抓並覆蓋 | `HistoryWindow` 不看既有資料；存入本來就覆蓋同一個開盤時刻 |
