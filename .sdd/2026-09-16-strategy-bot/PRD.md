...
- **Problem Statement:**
  ...
  **沿著那條路走的只有使用者自己打的測試訊息**。系統仍然沒有任何一件事，
  ...
  同時，一位使用者手上的策略腳本是**各說各話的**：每一支各自吐一個信號，
...
- **Primary Role(s):** **使用者**——這台操作台的主人。他已經有幾支自己寫的、
  或從策略腳本市集採用來的策略腳本，也已經完成了自己的 Telegram 投遞設定。
  系統沒有角色之分，沒有管理員。
...
### US-01 — 把幾支策略腳本組成一台機器人 [priority: P0]

**As a** 使用者，**I want** 把我手上幾支策略腳本的信號，用「且」與「或」組成買入與賣出兩個條件，
**so that** 「甲說買、乙說賣的時候該怎麼辦」這個判斷被寫下來，而不是每次自己重想一遍。

```gherkin
Scenario: 組起一台機器人
  Given 這位使用者的可用策略腳本裡有策略腳本甲與策略腳本乙
  When 他建立一台機器人,交易標的是 BTCUSDT
  And 宣告信號來源 A 為策略腳本甲、一小時刻度
  And 宣告信號來源 B 為策略腳本乙、五分鐘刻度
  And 買入條件為「A 等於買入 且 B 等於買入」
  And 賣出條件為「A 等於賣出」
  Then 機器人建立成功,並回覆它的機器人識別碼
  And 這台機器人的執行狀態是已停止

Scenario: 同一支策略腳本以不同參數值當成兩個來源
  Given 策略腳本甲宣告了一個參數「回看根數」
  When 他宣告信號來源 A 為策略腳本甲、回看根數 20
  And 宣告信號來源 B 為策略腳本甲、回看根數 60
  Then 機器人建立成功
...
Scenario: 參數名稱對不上
  Given 策略腳本甲只宣告了參數「回看根數」
  When 他宣告信號來源 A 為策略腳本甲,並給定參數「週期」的值
  Then 建立被拒絕,並顯示參數名稱「週期」對不上策略腳本甲宣告的參數

Scenario: 信號來源指名一支看不到的策略腳本
  Given 有一支策略腳本既不屬於這位使用者,也沒有被發佈到策略腳本市集
  When 他宣告一個信號來源指名那一支
  Then 建立被拒絕,並顯示「找不到」
  And 這句話與指名一支不存在的策略腳本時所顯示的一字不差
...
**so that** 我收到的是一個已經想過的意見，而不是三支策略腳本各說各話。
...
Scenario: 策略腳本被擁有者刪了就停下這台機器人
  Given 這台機器人正在執行中
  And 某一個信號來源指的策略腳本已經被刪除
  When 跑一輪
  Then 這台機器人變成已停止
  And 它的停擺原因是那一支策略腳本找不到了
  And 不送出任何訊息

Scenario: 採用來的策略腳本被原作者取消發佈
  Given 這台機器人正在執行中
  And 某一個信號來源指的是採用來的策略腳本,而原作者已經取消發佈
  When 跑一輪
  Then 這台機器人變成已停止
  And 它的停擺原因與策略腳本被刪除時一字不差,都是那一支策略腳本找不到了

Scenario: 算式執行失敗就停下這台機器人
  Given 這台機器人正在執行中
  And 某一支策略腳本的指標算式執行失敗
  When 跑一輪
  Then 這台機器人變成已停止
  And 它的停擺原因是那一支策略腳本算不出來
...
flowchart TD
    ...
    C -- 是 --> D[每一個信號來源各跑一次]
    D --> E{每一支都算出信號了嗎}
    E -- 策略腳本找不到 / 算式失敗 --> S[停下這台機器人並留下停擺原因]
...
| 機器人名稱長度上限 | **128 個字** | 比照策略腳本名稱，同一條規則不生第二個數字 |
...
| 停擺原因 | 什麼時候 | 為什麼要停 |
| :--- | :--- | :--- |
| **某一支策略腳本找不到了** | 策略腳本被擁有者刪除，或採用來的策略腳本被原作者取消發佈 | 再跑一萬次都是同一個結果 |
| **某一支策略腳本算不出來** | 指標算式執行失敗，或取用了沒宣告的參數名稱 | 同上——算式壞了不會自己好 |
| **機器人金鑰不被接受** | Telegram 拒絕這組金鑰 | 要重填金鑰，不是等一等 |
| **找不到這個聊天室** | Telegram 說沒有這個聊天室 | 要重填代號，不是等一等 |
| **沒有 Telegram 投遞設定** | 擁有者在它執行中把設定整份移除了 | 沒有壞掉，但等也等不到——一台沒地方講話的機器人，執行中與已停止之間沒有任何差別，而那正是當初拒絕啟動它的理由 |
...
- **兩個信號來源指同一支策略腳本、同一組參數值**：允許，但它們是兩個來源，各跑各的。
...
- **Security:**
  ...
  - **指標算式從頭到尾不離開系統**：機器人交出的信號來源只說「指名哪一支策略腳本」，
    **不含指標算式**——採用來的別人的策略腳本尤其如此。
  - 看不到一台機器人的每一種理由共用同一句「找不到」，一字不差；
    分開講等於讓任何人逐一試探識別碼就問得出系統裡有哪些機器人存在。

- **Compatibility:**
  - 沿用既有的六種彙總刻度、五種指標值種類、三個信號值，一個都不新增。
  - **機器人訊息不受測試訊息的 4096 字上限約束。** 那條上限的用處是讓**打字的人**
    知道自己送出去的不是半則；一則系統自己組出來的訊息若因為太長被拒，結果是
    **一台機器人為了自己的訊息而靜默**，而那個拒絕是說給一個不在現場的人聽的。
    機器人訊息的長度由信號來源上限（10 個）約束；組得出超長訊息是設計錯了，
    不是執行期該拒絕的事。
